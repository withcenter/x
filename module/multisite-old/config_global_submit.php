...
meta( 'title' , $in['title'] );
meta( 'secondary_title' , $in['secondary_title'] );
meta( 'footer_text' , $in['footer_text'] );

for( $i = 1; $i <= 10; $i++ ) {
	meta( 'forum_no_'.$i , $in['forum_no_'.$i] );
}

meta( 'theme_sidebar' , $in['theme_sidebar'] );
...
meta( 'img_url' , stripslashes(x::site_url(etc::domain())).'/'.G5_DATA_DIR.'/upload/multisite/'.$site[0].'/' );

foreach( $_FILES as $key => $value ) {
	if( $in[$key.'_remove'] == 'y' ) meta( $key , '' );							// if checkbox( file-upload-name_remove ) == 'y' 
