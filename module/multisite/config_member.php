<?php
	if ( $in['option'] == 'mb_edit' ) include_once 'config_member_edit.php';
		...
		$total_post = db::result ( "SELECT COUNT(*) FROM ".$g5['member_table']." $cond" );
		$rows = db::rows("SELECT * FROM ".$g5['member_table']." $cond ORDER BY mb_no DESC LIMIT $start, $no_of_post");
?>
	<div class='search-form'>
		<form method='get'>
			...
			<span>아이디/닉네임 <input type="name" name="username" value="<?=$username?>" /></span>
	...
	<div class='no-of-member'>검색 회원수 <?=$total_post?></div>
